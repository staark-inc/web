...
export default async function ContactsPage({ searchParams }: PageProps) {
  ...
  const filter: ContactFilter = CONTACT_FILTERS.includes(
  ...
  const filteredContacts = contacts.filter((contact) => {
    ...
    if (filter === "leads") {
      return contact._count.leads > 0;
    }

    if (filter === "clients") {
      return contact._count.clients > 0;
    }

    if (filter === "unlinked") {
      return contact._count.leads === 0 && contact._count.clients === 0;
    }

    return true;
  });

  const contactsWithLeads = contacts.filter(
    (contact) => contact._count.leads > 0
  ).length;
  const contactsWithClients = contacts.filter(
    (contact) => contact._count.clients > 0
  ).length;
  const totalMessages = contacts.reduce(
    (total, contact) => total + contact._count.messages,
    0
  );
    ...
    <div className="hub-page hub-contacts-v2-page">
      <header className="hub-contacts-v2-head">
        <div>
          ...
          <h1>Contacts</h1>
          <p>
            Track everyone who has reached Staark Inc. and see where each
      ...
      <section className="hub-contacts-v2-stats" aria-label="Contact overview">
        <div className="hub-contacts-v2-stat">
          <span className="hub-contacts-v2-stat-icon">
            <UserRound size={16} />
          </span>
          <div>
        ...
        <div className="hub-contacts-v2-stat">
          <span className="hub-contacts-v2-stat-icon hub-contacts-v2-stat-icon-lead">
            <Target size={16} />
          </span>
          <div>
            <small>With leads</small>
            <strong>{contactsWithLeads}</strong>
            <span>Commercial interest</span>
          </div>
        </div>

        <div className="hub-contacts-v2-stat">
          <span className="hub-contacts-v2-stat-icon hub-contacts-v2-stat-icon-client">
            <Building2 size={16} />
          </span>
          <div>
            <small>Linked clients</small>
            <strong>{contactsWithClients}</strong>
            <span>Converted relationships</span>
          </div>
        </div>
      ...
      <section className="hub-contacts-v2-tools" aria-label="Contact filters">
        ...
        <nav className="hub-contacts-v2-filters" aria-label="Contact status">
          <Link
            href={getFilterHref("all", q)}
            className={
              filter === "all" ? "hub-contacts-v2-filter-active" : undefined
            }
          >
            All
            <strong>{contacts.length}</strong>
          ...
          <Link
            href={getFilterHref("leads", q)}
            className={
              filter === "leads" ? "hub-contacts-v2-filter-active" : undefined
            }
          >
            Leads
            <strong>{contactsWithLeads}</strong>
          ...
          <Link
            href={getFilterHref("clients", q)}
            className={
              filter === "clients" ? "hub-contacts-v2-filter-active" : undefined
            }
          >
            Clients
            <strong>{contactsWithClients}</strong>
          ...
          <Link
            href={getFilterHref("unlinked", q)}
            className={
              filter === "unlinked" ? "hub-contacts-v2-filter-active" : undefined
            }
          >
            Unlinked
          </Link>
      ...
      {filteredContacts.length === 0 ? (
        <div className="hub-contacts-v2-empty">
          ...
          <p>
            {contacts.length === 0
              ? "New contacts will appear here after the first enquiry or conversation."
              : "Try another search term or switch the current filter."}
          </p>

          {contacts.length > 0 ? (
            <Link href="/hub/contacts" className="hub-secondary-button">
              Clear filters
            </Link>
          ) : null}
        </div>
          ...
          {filteredContacts.map((contact) => (
            <article key={contact.id} className="hub-contacts-v2-row">
              <div className="hub-contacts-v2-main">
                <div className="hub-contacts-v2-avatar">
              ...
              <div className="hub-contacts-v2-signals">
                <span
                  className={
                    contact._count.leads
                      ? "hub-contacts-v2-signal-lead"
                      : undefined
                  }
                >
                  <Target size={12} />
                  {contact._count.leads} lead
                  {contact._count.leads === 1 ? "" : "s"}
                </span>

                <span
                  className={
                    contact._count.clients
                      ? "hub-contacts-v2-signal-client"
                      : undefined
                  }
                >
                  <Building2 size={12} />
                  {contact._count.clients
                    ? `${contact._count.clients} client${
                        contact._count.clients === 1 ? "" : "s"
                      }`
                    : "Not a client"}
                </span>
                ...
                <span>
                  ...
                  {contact._count.messages} message
