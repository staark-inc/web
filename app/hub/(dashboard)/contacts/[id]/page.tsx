...
export default async function ContactDetailPage({ params, searchParams }: PageProps) {
  ...
  const messages = contact.email
    ? await prisma.message.findMany({
        where: {
          OR: [{ fromEmail: contact.email }, { toEmail: contact.email }],
        },
  ...
  const lastActivity = lastActivityCandidates.sort((a, b) => b.getTime() - a.getTime())[0];
  const displayName = contact.name || contact.company || contact.email || "Unknown contact";
  ...
  return (
    <div className="hub-page hub-contact-v2-detail-page">
      ...
      <header className="hub-contact-v2-hero">
        <div className="hub-contact-v2-avatar">{getInitials(contact.name, fallback)}</div>
        ...
        <div className="hub-contact-v2-actions">
          ...
          <Link href={`/hub/contacts/${contact.id}/edit`} className="hub-secondary-button">
            <Pencil size={14} /> Edit
          </Link>
        </div>
      </header>
      ...
      <section className="hub-contact-v2-stats" aria-label="Contact summary">
        <div>
          <Target size={15} />
          <span><small>Leads</small><strong>{contact.leads.length}</strong></span>
        </div>
        <div>
          <MessageSquareText size={15} />
          <span><small>Messages</small><strong>{messages.length}</strong></span>
        </div>
        <div>
          <Building2 size={15} />
          <span><small>Client status</small><strong>{contact.clients.length ? "Linked" : "Not linked"}</strong></span>
        </div>
        <div>
          <Clock3 size={15} />
          <span><small>Last activity</small><strong>{lastActivity ? formatDate(lastActivity) : "—"}</strong></span>
        </div>
      </section>
      ...
      <div className="hub-contact-v2-grid">
        <section className="hub-contact-v2-panel">
          <div className="hub-contact-v2-panel-head">
            <div><span>PROFILE</span><h2>Contact information</h2></div>
            <UserRound size={16} />
          </div>
          ...
          <div className="hub-contact-v2-info">
            <div>
              ...
              <span>
                <small>Email</small>
                {contact.email ? <a href={`mailto:${contact.email}`}>{contact.email}</a> : <strong>Not specified</strong>}
              </span>
            </div>
            ...
            <div>
              ...
              <span>
                <small>Phone</small>
                {contact.phone ? <a href={`tel:${contact.phone}`}>{contact.phone}</a> : <strong>Not specified</strong>}
              </span>
            </div>
            ...
            <div>
              ...
              <span>
                ...
                {contact.facebook ? (
                  <a href={contact.facebook} target="_blank" rel="noreferrer">Open profile</a>
                ) : (
            ...
            <div>
              <Building2 size={15} />
              <span><small>Company</small><strong>{contact.company || "Not specified"}</strong></span>
            </div>
          </div>
            ...
            <div className="hub-contact-v2-clients">
              ...
              {contact.clients.map((client) => (
                <Link key={client.id} href={`/hub/clients/${client.id}`}>
                  <Building2 size={14} />{client.name}<ArrowRight size={13} />
                </Link>
              ))}
            </div>
          ) : null}
        ...
        <section className="hub-contact-v2-panel">
          <div className="hub-contact-v2-panel-head">
            <div><span>PIPELINE</span><h2>Lead history</h2></div>
            <strong>{contact.leads.length}</strong>
          </div>

          {contact.leads.length === 0 ? (
            <div className="hub-contact-v2-panel-empty"><Target size={20} /><p>No leads for this contact yet.</p></div>
          ) : (
            <div className="hub-contact-v2-leads">
              {contact.leads.map((lead) => (
                <Link key={lead.id} href={`/hub/leads/${lead.id}`}>
                  ...
                  <span className={`hub-lead-status hub-lead-status-${lead.status.toLowerCase()}`}>{lead.status}</span>
                </Link>
              ))}
            </div>
          )}
      ...
      <section className="hub-contact-v2-panel hub-contact-v2-messages-panel">
        <div className="hub-contact-v2-panel-head">
          <div><span>COMMUNICATION</span><h2>Message history</h2></div>
        ...
        {messages.length === 0 ? (
          <div className="hub-contact-v2-panel-empty">
            ...
            <p>{contact.email ? "No messages with this contact." : "Email is not set for this contact."}</p>
          </div>
        ) : (
          <div className="hub-contact-v2-messages">
            {messages.map((message) => (
              <Link
                key={message.id}
                href={message.direction === "INBOUND" ? `/hub/message/${message.id}` : `/hub/sent/${message.id}`}
              >
                <span className={`hub-contact-v2-message-icon hub-contact-v2-message-icon-${message.direction.toLowerCase()}`}>
                  <Mail size={13} />
                </span>
                <div className="hub-contact-v2-message-copy">
                  <strong>{message.subject || "(No subject)"}</strong>
                  <span>{message.direction === "INBOUND" ? `From ${displayName}` : "Sent by Staark Inc."}</span>
                </div>
