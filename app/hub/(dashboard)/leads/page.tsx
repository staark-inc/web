    ...
    <div className="hub-page hub-leads-v2-page">
      <header className="hub-leads-v2-head">
        <div>
          ...
          <h1>Leads</h1>
          <p>
            Track new enquiries from first contact through qualification and conversion.
          </p>
        </div>
      ...
      <section className="hub-leads-v2-stats" aria-label="Sales pipeline overview">
        <div className="hub-leads-v2-stat">
          <span className="hub-leads-v2-stat-icon hub-leads-v2-stat-icon-new">
            <Target size={16} />
          </span>
          <div>
            <small>New</small>
            <strong>{newCount}</strong>
            <span>Needs first contact</span>
          </div>
        </div>

        <div className="hub-leads-v2-stat">
          <span className="hub-leads-v2-stat-icon">
        ...
        <div className="hub-leads-v2-stat">
          <span className="hub-leads-v2-stat-icon hub-leads-v2-stat-icon-qualified">
        ...
        <div className="hub-leads-v2-stat">
          <span className="hub-leads-v2-stat-icon hub-leads-v2-stat-icon-won">
            <Trophy size={16} />
          </span>
          <div>
            <small>Won</small>
            <strong>{wonCount}</strong>
            <span>{decidedCount ? `${winRate}% of decided leads` : "No decisions yet"}</span>
          </div>
        </div>
      </section>
        ...
        {filters.map((filter) => {
          ...
          return (
            <Link
              key={filter.value}
              href={href}
              className={active ? "hub-leads-v2-filter-active" : undefined}
            >
      ...
      {leads.length === 0 ? (
        ...
        <section className="hub-leads-v2-list" aria-label="Lead pipeline">
          {leads.map((lead) => {
            const initials = getInitials(lead.contact.name, lead.contact.email);
            ...
            return (
              <article key={lead.id} className="hub-leads-v2-row">
                ...
                <div className="hub-leads-v2-tail">
                  {lead.client ? (
                    <Link
                      href={`/hub/clients/${lead.client.id}`}
                      className="hub-leads-v2-client-link"
                    >
                  ...
                  <Link href={`/hub/leads/${lead.id}`} className="hub-leads-v2-open">
                    Open
                    <ArrowRight size={13} />
                  </Link>
                </div>
